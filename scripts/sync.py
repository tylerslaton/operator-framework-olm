...
def clone_repository(repo, fork_repo):
    try:
        repo.clone(path=fork_repo)
    except:
        print(f"Failed to clone repo {fork_repo}")
        raise
...
def sync(token, args):
    ...
    # TODO: running into issues where the git.Repo call is failing.
    # not too sure how to handle this yet without first cloning the
    # repository, but I'm probably misunderstading and/or misusing
    # this method entirely.
    print(fork_repo)

    repo = git.Repo(search_parent_directories=True)
    fork_repo = "github.com/timflannagan/operator-framework-olm"

    if args.clone_repository == "true":
        print("Cloning the repository")
        clone_repository(repo, fork_repo)
